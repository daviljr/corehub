...
export default async function Home() {
  ...
  // derive categories from products (category_id or categories)
  ...
  const categories = Object.values(categoriesMap).slice(0, 8);
    ...
    <div className="space-y-10">
      <section className="rounded-lg p-8 bg-gradient-to-r from-emerald-600 to-teal-500 text-white shadow-xl">
        <div className="max-w-6xl mx-auto">
          ...
          <p className="mt-3 max-w-3xl text-lg text-emerald-100">Escolha produtos premium e pacotes de viagem com conforto, segurança e atendimento humano.</p>
          <div className="mt-6 flex flex-wrap gap-3">
            <Link href="/store" className="inline-block bg-white text-teal-700 px-5 py-2 rounded-md font-semibold shadow">Ver Loja</Link>
            <a href="#destaques" className="inline-block px-5 py-2 border border-white/30 rounded-md text-white/90">Destaques</a>
          </div>
        </div>
      </section>
      ...
      {categories.length > 0 && (
        <section className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-2 md:grid-cols-6 gap-4">
            {categories.map((c) => (
              <Link key={c.id} href={`/store?category=${encodeURIComponent(c.slug)}`} className="block p-4 bg-white border rounded shadow-sm hover:shadow-md transition text-center">
      ...
      <section id="destaques" className="max-w-6xl mx-auto">
        <div className="flex items-center justify-between mb-4">
          ...
          <Link href="/store" className="text-sm text-slate-600">Ver todos</Link>
        </div>
      ...
      <section className="max-w-6xl mx-auto rounded-lg border p-6 bg-white shadow-sm">
        <div className="flex flex-col md:flex-row items-center justify-between gap-4">
          ...
          <div>
            <Link href="/contact" className="inline-block px-5 py-2 bg-emerald-600 text-white rounded-md">Contato</Link>
          </div>
        </div>
      </section>
    </div>
